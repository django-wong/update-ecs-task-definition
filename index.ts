...
async function _run() {
    ...
    const forceNewDeployment = core.getBooleanInput(
        "force-new-deployment",
        O_T
    );

    const waitForService = core.getBooleanInput(
    ...
    res.taskDefinition?.containerDefinitions?.forEach((container) => {
        if (
            (containerName &&
                (container.name ?? "").match(new RegExp(containerName))) ||
            !containerName
        ) {
    ...
    const minutes = parseInt(waitForMinutes || "15");
